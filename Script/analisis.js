...
function medianaEmpresaYear(nombre, year) {
    if (!empresas[nombre]) {
        ...
        return FuncionesMatematicas.calcularMediana(empresas[nombre][year]);
    }
}
...
function proyeccionPorEmpresa(nombre) {
    ...
    if (!empresas[nombre]) {
        ...
        //obtenemos los years que tienen salarios 
        const empresaYear = Object.keys(empresas[nombre]);
        console.log(empresaYear);
        const listaMedianaYear = empresaYear.map(year => medianaEmpresaYear(nombre, year));
        // recibimos las medianas [800,4541,545]
          //console.log(listaEmpresaYear);  
        ...
        //una vez que tenemos la mediana tenemos que multiplicar el resultado con el ultimo salario 
        //para saber el aumento         trabajps.length-1 por que es la poscicion del ultimo salario
        const ultimoMediana = listaMedianaYear[listaMedianaYear.length - 1];
